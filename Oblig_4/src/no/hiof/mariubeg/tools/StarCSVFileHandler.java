package no.hiof.mariubeg.tools;

import no.hiof.mariubeg.interfaces.ObjectFileHandler;
        ...
        try (BufferedReader bufferedReader = new BufferedReader(new FileReader(file))) {
            ...
            while ( (line = bufferedReader.readLine()) != null) {
                if (!line.equalsIgnoreCase("name,radius,mass,effectiveTemp")) {
                    ...
                    stars.add(new Star(values[0], Double.parseDouble(values[1]), Double.parseDouble(values[2]), Double.parseDouble(values[3])));
                }
